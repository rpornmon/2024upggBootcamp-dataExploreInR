...
Are they in your /Downloads? If yes, no worries. Let's practice moving them to the ./raw_data directory.

```{bash example file move}
mv /Downloads/GSE81932_Dataset01.txt.gz /Desktop/2024upggBootcamp-dataExploreinR
mv /Downloads/GSE81932_Dataset02.txt.gz /Desktop/2024upggBootcamp-dataExploreinR
mv /Downloads/ribosome_biogenesis_annotations.txt /Desktop/2024upggBootcamp-dataExploreinR
mv /Downloads/doi_10_5061_dryad_d644f__v20160422.zip /Desktop/2024upggBootcamp-dataExploreinR
```

Let's extract it all now. 
You could do this using some commands you learned yesterday! Or, we can do it in R:

```{r load data}
library(tidyverse)
mRNA_file <- unzip("../raw_data/GSE81932_Dataset01.txt.gz", files = "GSE81932_Dataset01.txt")
mRNA_data <- read_lines(mRNA_file)

periodically_expressed_genes_file <- unzip("../raw_data/GSE81932_Dataset02.txt.gz", files = "GSE81932_Dataset02.txt")
...
scer_names_estimates_file <- "../raw_data/scer-mrna-protein-absolute-estimate.txt"
```
